...
$(function() {
				...
				if (frmcompdet.form()) {
					...
					$.ajax({
						...
						success: function(response) {
							...
							if (response.class == "succ") {
								messageContainer.append('<div class="'+response.class+'">'+response.message+'</div>').show();
							} else {
								$('#loader').hide();
								$.each(response, function(idx, obj) {
				...
				if (frmbusdet.form()) {
					...
					$.ajax({
						...
						success: function(response) {
							...
							if (response.class == "succ") {
								$("#frmbusdet .message-container").append('<div class="'+response.class+'">'+response.message+'</div>').show();
							} else {
								$('#loader').hide();
								$.each(response, function(idx, obj) {
									messageContainer.append('<div class="err">'+obj+'</div>');
			...
			if (frmbankdet.form()) {
				...
				$.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.class == "succ") {
                            messageContainer.html('<div class="'+response.class+'">'+response.message+'</div>').show();
                        } else {
                            $('#loader').hide();
                            $.each(response, function(idx, obj) {
                                messageContainer.append('<div class="err">'+obj+'</div>');
				...
				$.ajax({
					...
					success: function(response) {
						if (response.class=="succ") {
							...
							$('#image-preview-user').attr('src', base_url+'/public/uploads/profile/'+response.image_name);
						} else {
							$('#loader').hide();
							$.each(response, function(idx, obj) {
								console.log(obj);
	...
	$(document).ready(function(){
		...
		$("body").on("click", ".AddMore", function() {
			...
			clonedContainer.insertBefore(this).append($('<button type="button" name="remove" id="' + i + '" class="btn btn-sm btn-outline-danger btn_remove py-1 mb-2">X</button>'));
		});
...
button to the new container
			newContainer.append(
				$('<button type="button" name="remove" id="' + i + '" class="btn btn-sm btn-outline-danger btn_remove_partner py-1 mb-2">X</button>')
			);
		
			// Insert the new container before the AddMorePartner button
