...
//------------ Company Profile empty check ----------
function CheckCompanyDetails($request)
{
    $userId = Auth::id();
	...
	if (empty($companyProfile) || empty($companyProfile->comp_name) || empty($companyProfile->comp_nature) ) {
        $request->session()->flash('alert', 'Please complete your company profile.');
        return true;
    }
...
Route::get('/companyprofile', 'CompanyProfileController@index')->name('companyprofile.index');

Route::middleware(['auth'])->group(function () {
    Route::get('/', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return redirect()->route('customers.index');
    });

    Route::get('/sales-invoice', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\SalesController@salesInvoiceIndex');
    })->name('sales.invoice.index');

    Route::get('/add-sales-invoice', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\SalesController@addSalesInvoice');
    })->name('sales.invoice.add');

    Route::get('/sales-credit-debit', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\SalesController@CreditDebitIndex');
    })->name('sales.creditdebit.index');

    Route::get('/add-sales-credit-debit', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\SalesController@addCreditDebit');
    })->name('sales.creditdebit.add');

    Route::get('/charterd', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\ChaterdController@index');
    })->name('charterd.index');
    ...
    Route::get('/items', function (Illuminate\Http\Request $request) {
        ...
        return app()->call('App\Http\Controllers\ItemsController@index');
    })->name('items.index');

    Route::get('/gst', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\GSTController@index');
    })->name('gst.index');

    Route::get('/purchase-invoice', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\PurchaseController@PurchaseInvoiceIndex');
    })->name('purchase.invoice.index');

    Route::get('/purchase-credit-debit', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\PurchaseController@CreditDebitIndex');
    })->name('purchase.creditdebit.index');

    Route::get('/liabilities', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\LiabilitiesController@index');
    })->name('liabilities.index');

    Route::get('/show-assets', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\FixedAssetController@index');
    })->name('show.assets.index');

    Route::get('/asset-voucher', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\FixedAssetController@AssetVoucherIndex');
    })->name('asset.voucher.index');

    Route::get('/payment', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\PaymentController@index');
    })->name('payment.index');
    ...
    Route::get('/expenses', function (Illuminate\Http\Request $request) {
        ...
        return app()->call('App\Http\Controllers\ExpensesController@index');
    })->name('expenses.index');
    ...
    Route::get('/banks', function (Illuminate\Http\Request $request) {
        ...
        return app()->call('App\Http\Controllers\CashController@BankIndex');
    })->name('banks.index');

    Route::get('/loans', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\CashController@LoanIndex');
    })->name('loans.index');

    Route::get('/cash', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\CashController@CashIndex');
    })->name('cash.index');

    Route::get('/vendors', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
    ...
    Route::get('/statutory', function (Illuminate\Http\Request $request) {
        // if (CheckCompanyDetails($request)) {
        //     return redirect()->route('companyprofile.index');
        // }
        return app()->call('App\Http\Controllers\StatutoryController@statutory');
    })->name('statutory.index');

    Route::get('/profit-loss', function (Illuminate\Http\Request $request) {
        if (CheckCompanyDetails($request)) {
            return redirect()->route('companyprofile.index');
        }
        return app()->call('App\Http\Controllers\ReportController@profit_loss');
...
Route::group(['middleware' => ['auth','SubscriberStatus']],function(){
	...
	Route::get('getinvcust_purchase', 'PurchaseController@getinvcust_purchase'); // added 13-07-2024 khokan
	//End of Purchase Controller

	//Asset Controller
	// Route::get('/show-assets', 'FixedAssetController@index');
	...
	Route::get('/addcoupons', 'CouponsController@addCoupon');
	Route::post('/save_add_coupon', 'CouponsController@save_add_coupon');
	Route::get('/edit-coupon/{couponId}', 'CouponsController@edit_coupon');
	Route::post('/update_coupon', 'CouponsController@update_coupon');
	Route::get('delCoupon', 'CouponsController@delCoupon');
	...
	Route::post('/save-income', 'IncomeController@store')->name('income.store');
	Route::get('/view-income/{Id}', 'IncomeController@getViewIncome');
	Route::get('/edit-income/{Id}', 'IncomeController@editIncome');
	Route::post('/update-income', 'IncomeController@updateIncome')->name('income.update');


	//End of Income Controller
	...
	//Customer Assignment
	Route::post('/comp_check_email', 'ClientController@comp_check_email'); 
	...
	Route::get('/assignment-details/{clientId}', 'CustomerAssignmentController@AssignmentDetails');
	//Employee
	...
	//Reminder
	Route::get('/remider_from_ca', 'ReminderController@remider_from_ca');
	//Payments
	...
	//date 07-06-2024
	Route::post('/fetchSalesItem', 'SalesController@fetchSalesItem');
	Route::post('/fetchPurchaseItem', 'PurchaseController@fetchPurchaseItem');
...
//---- Added 31-07-2024 -------------
Route::post('/userListsAccess', 'ReminderController@userListsAccess');
Route::post('/sendReminderCA', 'ReminderController@sendReminderCA');
Route::get('/payment_invoice_customer/{payId}/{cId}', 'ClientController@payment_invoice_customer');

Route::get('/get-bank-details', 'UserDashboardController@getBankDetails');
Route::get('/get-monthly-data', 'UserDashboardController@getMonthlyData');
Route::get('/get-Statutory-data', 'UserDashboardController@getStatutoryData');
Route::get('/get-customer-data', 'UserDashboardController@getCustomerData');
Route::get('/get-monthlyturnover-data', 'UserDashboardController@getMonthlyturnoverData');
Route::get('/get-receivables-data', 'UserDashboardController@getReceivablesData');
Route::get('/get-payables-data', 'UserDashboardController@getPayablesData');

Route::post('/apply-coupon', 'UserDashboardController@applyCoupon')->name('apply.coupon');


//------------ Ca Dashboard ---------------

Route::get('/get-customer-payment-status', 'CaDashboardController@getCustomerPayment');
Route::get('/get-task-status', 'CaDashboardController@getTaskStatus');
Route::get('/get-task-client', 'CaDashboardController@getTaskClient');
Route::get('/get-monthWish-client', 'CaDashboardController@getMonthWishClient');
Route::get('/get-monthWish-payment', 'CaDashboardController@getMonthWishPayment');
