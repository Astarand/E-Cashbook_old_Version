...
class EmployeeController extends Controller
{
    ...
    public function Index()
    {
		...
		return view('pages.ca.employee')->with([
			'title' =>$title,
			'employees'=>$employees,
	...
	public function AddEmployee()
    {
		...
		return view('pages.ca.addemployee')->with([
			'countries'=>$countries,
			'depts' => $depts
		]);
    }
	...
	public function edit_employee($empId)  {  
		...
		$employee = DB::table('users')
					...
					->leftJoin('employees', 'users.id', '=', 'employees.empId')
					->where('users.u_type','=',4) 
					->where('users.id', '=', $empId) // ------------- Add new 10-06-2024 
					->where('employees.added_by','=',$userId)
					->get();
		...
		$desigs = Designations::where('dept_id', '=', $employee->dept_id)->get();
		 ...
		 return view('pages.ca.edit-employee')->with([
				'countries'=>$countries,
				'c_states'=>$c_states,
				'c_cities'=>$c_cities,
				'p_states'=>$p_states,
	...
	public function view_employee($empId)  {  
		...
		$desigs = Designations::where('dept_id', '=', $employee->dept_id)->get();
		 ...
		 return view('pages.ca.view-employee')->with([
				'countries'=>$countries,
				'c_states'=>$c_states,
				'c_cities'=>$c_cities,
				'p_states'=>$p_states,
